...
export default function EditCommunityForm({ post }: EditFormProps) {
    ...
    const [meetupLocation, setMeetupLocation] = useState(post.meetupLocation || '');
    const [meetupDate, setMeetupDate] = useState(
        post.meetupDate ? format(new Date(post.meetupDate), "yyyy-MM-dd'T'HH:mm") : ''
    );
    const [meetupPurpose, setMeetupPurpose] = useState(post.meetupPurpose || 'DRINK');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await updateCommunityPost(post.id, formData, session?.access_token);
